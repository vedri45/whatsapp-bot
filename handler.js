...
const handlePromptRequest = async (msg) => {
    ...
    if (!lastFetchTime || Date.now() - lastFetchTime >= waitTime) {
        try {
            ...
            if (text.length === 0) {
                await msg.reply('Sorry, the generated text is empty. Please try again with a different prompt.', msg.from);
                return;
            }

            if (text.length > 2000) {
                await msg.reply('Sorry, the generated text is too long. Please try again with a shorter prompt.', msg.from);
                return;
            }
            ...
            await msg.reply('Sorry, there was an error generating text. Please try again later.', msg.from);
        } finally {
            lastFetchTime = Date.now();
        }
    } else {
